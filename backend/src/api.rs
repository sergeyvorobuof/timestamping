...
impl<T: TransactionSend + Clone + 'static> PublicApi<T> {
    ...
    fn wire_timestamp_filename(self, router: &mut Router) {
        let timestamp:fn(&mut Request)-> <unknown> = move |req: &mut Request :&mut Request| -> IronResult<Response> {
            let content_hash: Hash = self.url_fragment(req, "content_hash")?;
            let timestamp: Option<<unknown>> = self.timestamp(&content_hash)?;
            self.ok_response(&json!(timestamp))
        };
        router.get(
            glob:"/v1/timestamps/filename/:content_hash",
            handler: timestamp,
            route_id: "get_timestamp_by_filename",
        );
    }
